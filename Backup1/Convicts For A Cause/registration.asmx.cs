...
namespace Convicts_For_A_Cause
{
    ...
    public class Registration  : System.Web.Services.WebService
    {
        ...
        public string RegisterConvict(string email, string name, Double amount)
        {
            ...
            PaymentDTO paymentResults = paymentsHelper.AddPayment(email, email, amount, "", "convict registration");

            JavaScriptSerializer js = new JavaScriptSerializer();

            string strJSON = js.Serialize("{'emailaddress': '" + dto.convict.Email + "'}");
            return strJSON;
        ...
        public string RegisterTeamLeader(string email, string name, Double amount, string registrationType)
        {
            ...
            PaymentDTO paymentResults = paymentsHelper.AddPayment(email, email, amount, "", "team registration");
            /**
             *   Need to add error handling into this method. 
             **/
            JavaScriptSerializer js = new JavaScriptSerializer();

            string strJSON = js.Serialize("{'teamcode':'" + convictResults.convict.TeamCode + "', 'emailaddress': '" + convictResults.convict.Email + "'}");
            return strJSON;
        ...
        public string RegisterTeamMember(string email, string name, Double amount, string registrationType, string teamCode)
        {
            ...
            PaymentDTO paymentResults = paymentsHelper.AddPayment(email, email, amount, "", "team member registration");
            /**
             *   Need to add error handling into this method. 
             **/
            JavaScriptSerializer js = new JavaScriptSerializer();
